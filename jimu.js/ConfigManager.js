...
function (declare, lang, array, dojoKernel, dojoConfig, topic, aspect, i18n, cookie, Deferred, all,
  xhr, script, require, jimuUtils, tokenUtils,
  ...
  clazz = declare(null, {
      ...
      if(this.urlParams.config) {
        ...
        portal.loadSelfInfo().then(lang.hitch(this, function(portalSelf){
          ...
          this._processSignIn(portalUrl).then(lang.hitch(this, function(){
            ...
            this._getAppConfigFromAppId(portalUrl, this.urlParams.id).then(function(appConfig){
    ...
    * ?appid=<123>, the id is portal/AGOL app id, which is created from portal/AGOL template.
          ...
          then get WAB appid, then get WAB app config, then merge config;
          > IF NOT in portal, we'll use the appid to get app data, then merge the data
          to WAB app config.
        How to check whether the app is in portal?
          By default, portal url in app config is empty. When XT builder create app,
          builder will set the portal url. But in portal/AGOL, builder doesn't set the
          portal url. So, if portal url is empty, we consider the app is in portal.
          However, the exception is launch stemapp directly. The solution is the XT builder
          will write "wab_portalurl" cookie for stemapp. So, if we find this cookie, we know
          the app is not in portal.
    ...
    ********************************************************/
          ...
          this._proesssWebTierAndSignin(appConfig).then(lang.hitch(this, function() {
            ...
            if(this.urlParams.appid){
              ...
              if(!this.isRunInPortal){
                //config has portal url means app created from XT
                def = new Deferred();
    ...
    _processWebTier: function(appConfig){
      ...
      if(appConfig.isWebTier){
        //because tokenUtils.isWebTierPort can't success if url is http
    ...
    _loadWidgetManifest: function(setting){
      ...
      xhr(url, {
        ...
        var _processManifest = lang.hitch(this, function(){
        ...
        //manifest.json doesn't have i18nLabels, so load
        //nls/strings.js to fill in
        manifest.category = 'widget';
        manifest.amdFolder = setting.amdFolder;
        if(!setting.label){
          jimuUtils.addI18NLabel(manifest).then(function(){
            _processManifest();
          });
        }else{
          _processManifest();
    ...
    _checkLocale: function(){
      //if portalLocale is empty, use browser locale
      var portalLocale = this.portalSelf.user && this.portalSelf.user.culture ||
      this.portalSelf.culture || dojoConfig.locale;
      portalLocale = portalLocale.toLowerCase();

      if(jimuUtils.isLocaleChanged(dojoConfig.locale, portalLocale)){
        cookie('appbuilder_locale', portalLocale);
  ...
  function getCleanConfig(config){
    ...
    var properties = ['inPanel', 'isController', 'hasLocale', 'hasStyle', 'hasConfig',
    'hasUIFile', 'hasSettingPage', 'hasSettingUIFile', 'hasSettingLocale', 'hasSettingStyle',
