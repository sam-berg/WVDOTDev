  ...
  return declare([_BasicServiceBrowser], {
    ...
    _searchServiceUrlCallback: function(serviceUrl, serviceItem, serviceMeta){
      ...
      if(directLayers.length > 0){
        var isMapServer = serviceUrl.indexOf("/MapServer") >= 0;
        ...
        var supportAllLayers = isMapServer && currentVersion >= 10;
    ...
    _searchLayerUrl:function(layerUrl, parent, /* optional */ showError, /* optional */ definition){
      ...
      var callback = lang.hitch(this, function(definition){
        if(!this.domNode){
          return;
        }
        this.shelter.hide();
      ...
      if(definition){
        ...
        this.shelter.show();
        this._getRestInfo(layerUrl).then(callback, lang.hitch(this,function(err){
          console.error(err);
          if(!this.domNode){
            return;
          }
