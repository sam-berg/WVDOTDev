...
function(declare, lang, array, locale, esriLang, ItemFileWriteStore) {
  return declare([],{
    ...
    _preParseSingleExpr: function(_part) {
      ...
      var regIgnoreCaseLike = /^UPPER(.*)(\s+)LIKE(\s+)UPPER((.*))$/i;
      if (regIgnoreCaseLike.test(str)) {
        var nameStartIndex = str.indexOf('(');
        var nameEndIndex = str.indexOf(')');
        var fieldName = str.substring(nameStartIndex + 1, nameEndIndex);//CITY_NAME
        var valueStartIndex = str.lastIndexOf('(');
        var valueEndIndex = str.lastIndexOf(')');
        var value = str.substring(valueStartIndex + 2, valueEndIndex+1);//'#0#'
        ...
        this.storeValue(value, part);//value:'#0#' part.valueObj.value:#0#
        part.operator = "stringOperatorContainsCaseInSensitive";
        return part;
      }
    ...
    parseSingleExpr: function(part){
      //code for wab
      //start
      // var preParseResult = this._preParseSingleExpr(lang.clone(part));
      // if(preParseResult){
