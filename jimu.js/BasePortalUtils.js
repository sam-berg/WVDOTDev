...
define([
  ...
  function(declare, lang, array, dojoConfig, Deferred,
    topic, esriRequest, jimuUtils, portalUrlUtils) {
    ...
    var PortalClass = declare([], {
      ...
      signIn: function() {
        ...
        if (this.isValidCredential()) {
          ...
          def = this.tokenUtils.signInPortal(this.portalUrl);
        }
        ...
        if (this.user && this.user.declaredClass === 'jimu.PortalUser') {
          ...
          if (this.isValidCredential()) {
            if (this.credential.userId) {
              ...
              this.tokenUtils.getUserIdByToken(this.credential.token, this.portalUrl).then(
