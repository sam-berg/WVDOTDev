  ...
  return declare(LayerInfoForDefault, {
    ...
    loadLegends: function(portalUrl) {
      ...
      mapService.layerInfo.getLegendInfo(portalUrl).then(lang.hitch(this, function(legendInfo) {
        this.initLegendsNode(legendInfo);
      }));
    },

    initLegendsNode: function(legendInfo) {
      var mapService = this.originOperLayer.mapService;
      var loadingImg = query(".legends-loading-img", this._legendsNode)[0];
      if (legendInfo && loadingImg) {
        domConstruct.empty(this._legendsNode);
        //draw legend
