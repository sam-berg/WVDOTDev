  ...
  clazz = declare([_WidgetBase], {
    ...
    _onLayoutChange: function(appConfig){
      ...
      array.forEach(this.widgetManager.getPreloadPanelessWidgets(), function(widget){
        ...
        var position =
        utils.getPositionStyle(appConfig.getConfigElementById(widget.id).panel.position);
      ...
      array.forEach(this.preloadGroupPanels, function(panel){
        ...
        style = lang.mixin(style,
          utils.getPositionStyle(appConfig.getConfigElementById(panel.config.id).panel.position));
