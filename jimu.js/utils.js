...
function(lang, array, html, has, config, ioQuery, query, nlt, Deferred, on, json, cookie,
  ...
  mo.checkFileExist = function(file) {
    ...
    var filePath = require(mo.getRequireConfig()).toUrl(file);
    if(require.cache['url:' + filePath]){
      def.resolve(true);
      return def;
    }

    for(var p in i18n.cache){
      var f = file.substr(0, file.length - '.js'.length);
      if(p.startWith(f)){
        def.resolve(true);
        return def;
      }
  ...
  mo.setConfigByTemplateWithId = function(config, key, value) {
    ...
    function getSectionObject(sectionSearchStr) {
      ...
      if (sectionRange.firstPos !== -1) {
        ...
        var subKey   = key.slice(sectionRange.lastPos + 1, sectionRange.lastPos.length);
  ...
  function addWidgetManifestProperties(manifest) {
    ...
    var properties = ['inPanel', 'hasLocale', 'hasStyle', 'hasConfig', 'hasUIFile',
