...
define([
  ...
  CheckBox, PopupMenu, _TemplatedMixin, template, Legend, NlsStrings, domAttr, domClass, domStyle) {
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
      ...
      if(layerInfo.newSubLayers.length === 0) {
        ...
        if(this.config.showLegend) {
          ...
          domStyle.set(query(".showLegend-image",
                             layerInfo.layerNode.currentNode)[0],
                             'display',
                             'none');
        }
        return;
    ...
    addLayerNode: function(layerInfo, level, toTableNode) {
      ...
      }, toTableNode), layerTdNode, ckSelectDiv, ckSelect, imageShowLegendNode, imageNoLegendDiv,
      ...
      this.own(on(layerTrNode,
                  'click',
                lang.hitch(this, this._onRowTrClick, layerInfo, imageShowLegendNode, layerTrNode)));
      this.own(on(layerTrNode,
                'mouseover',
...
*/
      ...
      // layerInfo.initLegendsNode(layerInfo.legendsNode);
      try{
        var legendsNode = layerInfo.obtainLegendsNode();
    ...
    _fold: function(layerInfo, imageShowLegendNode) {
      if (domStyle.get(layerInfo.layerNode.subNode, 'display')  === 'none') {
        ...
        domAttr.set(imageShowLegendNode,
                    'src',
                    this.layerListWidget.folderUrl + 'images/v_right.png');
      }
    },
    ...
    _onRowTrClick: function(layerInfo, imageShowLegendNode, layerTrNode) {
      ...
      layerInfo.loadLegends(this.layerListWidget.appConfig.portalUrl);
    },
